...
def validate_peer_list():
    ...
    # print("! num_of_peers_threshold = ", num_of_peers_threshold)
    # print("! len(new_clientlist) = ", len(new_clientlist))
    # if len(client_state_list) > num_of_peers_threshold > len(new_clientlist):
    print("len(new_clientlist):", len(new_clientlist))
    print("num_of_peers_threshold: ", num_of_peers_threshold)
    if len(new_clientlist) < num_of_peers_threshold:
        # client_state_list = new_clientlist
        peers = new_clientlist
        ...
        if sorted(new_peers) != sorted(peers):
            print("**** [just to make you pay attention] **** ")
            print("len(peers):", len(peers))
            print("len(new_peers):", len(new_peers))
            # print("client_state_list:\n", client_state_list)
            # print("new_peers:\n", new_peers)
            peers.extend(new_peers)
            print("len(peers) now:", len(peers))
...
def write_to_file(piece_name, piece_index, total_piece_size):
    ...
    print(f"! recieved: piece_index = {piece_index}, downloaded: {round(len(my_bitfield_lst) * 100 / num_pieces_global, 2)}%")
    # time.sleep(1)
    ...
    # print(f"! bytes_Array = {piece_index}, offset = {offset}")
...
def add_bytes_to_dictionary(bytes, piece_index, total_piece_size, offset, client_we_are_serving, total_num_pieces, tracker):
    ...
    size_res = check_size(piece_index, total_piece_size,  total_num_pieces, client_we_are_serving)
    ...
    if(size_res == 1):
        ...
        if (piece_index == total_num_pieces - 1):
            ...
            if (last_piece_size - last_piece_size_requested) >= piece_len_per_request:   #if size of block we need, is larger than max request size, request max size
                ...
                if (last_piece_size - last_piece_size_requested) != 0:
                    #formatting to send
                    index = struct.pack("!I", piece_index)
                    begin = struct.pack("!I", last_piece_size_requested)
                    # print("!!! ", last_piece_size)
                    # print("!!! ", last_piece_size_requested)
                    ...
                    leftover = (last_piece_size - last_piece_size_requested)
                    length = struct.pack("!I", leftover)  #how much bytes are left for last piece (total_size - what we have recieved)
...
def check_size(piece_index, piece_size, total_pieces, client_we_are_serving):
    global piece_dictionary
    ...
    if(piece_name == -1):
        ...
        if(len(piece_dictionary[piece_name]) == piece_size):
            ...
            # print("Missing some blocks, waiting Almost there!! ...")
            return 2
...
def initialize_client_state_list():
    ...
    for p in peers:
        ...
        try:
            ...
            sd = socket.create_connection((p.peer_ip_addr, p.peer_port), timeout=0.1)
            # sd.settimeout(0.3)
            sd.settimeout(1)
            ...
            # print(f"could not connect to: {p.peer_ip_addr}:{p.peer_port}", e)
            socket_error = True


def request_last_piece(piece_index, p: Peer):
    global last_piece_size_requested
    # print("! request_last_piece")
    # print("piece_index:", piece_index)
    # print("last_piece_size:", last_piece_size)
    # print("last_piece_size_requested:", last_piece_size_requested)
...
def request_piece_random():
    random.shuffle(client_state_list)
    for c in client_state_list:
        my_bitfield_lst = convert_bitfield_to_list(my_bitfield)
        ...
        if bitfield_diff != []:
            for b in bitfield_diff:
                if (b not in current_requested_pieces_overall) and (len(c.current_requested_pieces) < 5):
                    if c.am_interested == 0 and c.peer_choking == 1:
                        print(f"! {c.peer_ip_addr} is choking us")
                        c.send_interested_msg()
                        c.am_interested = 1
                    elif c.am_interested == 1 and c.peer_choking == 0:
                        if b == (num_pieces_global - 1):
                            # print(f"! request_piece_random, requested piece idx {b} from {c.peer_ip_addr}")
...
def request_piece_rarest_first():
    top_3_at_most_rarest_pieces = []
    all_elements = []
    
    for c in client_state_list:
        if c.pieces_they_have == []:
            return
    ...
    most_common_pieces = Counter(all_elements).most_common()
    #Iterate through reverse order to get the least common pieces
    for i in range(len(most_common_pieces) - 1, max(0, len(most_common_pieces) - 3) - 1, -1):
        #print(i)
        p, _ = most_common_pieces[i]
        top_3_at_most_rarest_pieces.append(p)
        
    rarest_piece_to_request = random.choice(top_3_at_most_rarest_pieces)
    #print("Rarest Piece to Request", rarest_piece_to_request)
    #rarest_piece_to_request = num_pieces_global - 1
    
    for c in client_state_list:
        my_bitfield_lst = convert_bitfield_to_list(my_bitfield)
        bitfield_diff = list(set(c.pieces_they_have) - set(my_bitfield_lst))
        if (bitfield_diff != []) and c.current_requested_pieces == []:
            if rarest_piece_to_request in bitfield_diff:
                if (rarest_piece_to_request not in current_requested_pieces_overall):
                    if c.am_interested != 1 and c.peer_choking == 1:
                        c.send_interested_msg()
                        c.am_interested = 1
                    elif c.am_interested == 1 and c.peer_choking == 0:
                        if rarest_piece_to_request == (num_pieces_global - 1):
                            request_last_piece(rarest_piece_to_request,c)
                            ...
                            begin = struct.pack("!I", 0)
                            length = struct.pack("!I", piece_len_per_request)
                            # print(f"! request_piece_random, requested piece idx {rarest_piece_to_request} from {c.peer_ip_addr}")
                            c.send_request_msg(index, begin, length)
                            c.current_requested_pieces.append(rarest_piece_to_request)
                            current_requested_pieces_overall.append(rarest_piece_to_request)
...
if __name__ == '__main__':
    ...
    opt_unchoke_timer = 0
    top_four_timer = 0
    last_time_validate = datetime.now()
    last_time_clear = datetime.now()
    while True:
        ...
        # print("!!!!!!!!!!!!!!TOP FOUR - PEERS THAT ARE INTERESTED IN US!!!!!!!!!!!!!!!!")
        # print(top_four)
        if "top_4" in args.choking_strategy and len(top_four) >= 4:
            ...
            # print("!!!!!!!!!!!!!!TOP FOUR - PEERS W BEST DOWNLOAD RATES!!!!!!!!!!!!!!!!")
            # print(top_four)
            ...
            for c in client_state_list:
                ...
                if (c.peer_choking == 0) and (c.peer_interested == 0) and len(slower_peers) > 1:
                    # print(f"!!!!TOP FOUR - POTENTIAL CANDIDATE {c.peer_id} HAS BEEN UNCHOKED!!!!")
                    c.send_unchoke_msg()
        ...
        if (datetime.now() - last_time_clear).total_seconds() >= 30:
            last_time_clear = datetime.now()
            current_requested_pieces_overall.clear()
            for c in client_state_list:
                c.current_requested_pieces.clear()

        if (datetime.now() - last_time_validate).total_seconds() >= 60:
            last_time_validate = datetime.now()
            validate_peer_list()
        ...
        if rfds != []:
            for r in rfds:
                    ...
                    if (message_length == 0): #Keep Alive Message
                        ...
                        if id_num == 1: #Unchoke Message
                            ...
                            if client_we_are_serving.peer_last_send_time  == -1:
                                ...
                                # print("#######PEER DOWNLOAD RATE######")
                                # print(client_we_are_serving.peer_download_rate)
                                # print("######PEER LAST SEND TIME#####")
                                # print(client_we_are_serving.peer_last_send_time)
                ...
                #timePassed = currTime - lastTimeCheck 
                #keep_alive_timer += timePassed
            ...
            # print("IS ANYONE HOME RN???")
            for k in client_state_list:
                k.send_keep_alive_msg()

        #(Annie) Top-4 Choking Policy: Update the top-4
        currTime = datetime.now()
        timePassed = (currTime - lastTimeCheck).total_seconds() 
        opt_unchoke_timer += timePassed
        top_four_timer += timePassed
        
        # print("DO WE TOP FOUR UPDATE?")
        res =  "top_4" in args.choking_strategy and top_four_timer >= 10
        # print(res)
        # print(top_four_timer)
        if  "top_4" in args.choking_strategy and top_four_timer >= 10:
        ...
        # print("DO WE OPTIMISTIC UNCHOKE?")
        res =  "optimistic_unchoke" in args.choking_strategy and opt_unchoke_timer >= 30
        # print(res)
        # print(opt_unchoke_timer)
        if "optimistic_unchoke" in args.choking_strategy and opt_unchoke_timer >= 30:
            opt_unchoke_timer = 0
            # print("WE ARE OPTIMISTIC UNCHOKING")
            #Unchoke a single peer regardless of its upload rate
            unchoke_random = False
            while unchoke_random == False: #We are stuck in an infinite loop?
                print("loop loop")
                peer_idx = random.randint(0, len(client_state_list)-1)
                random_peer = client_state_list[peer_idx]
                if random_peer.am_choking == 0:
                    continue
                else:
                    # print("OPTIMISTIC UNCHOKE - We are unchoking a random peer")
                    random_peer.am_choking = 0
                    random_peer.send_unchoke_msg()
                    unchoke_random = True
                    if random_peer.peer_interested == 1 and "top_4" in args.choking_strategy:
                        # print("OPTIMISTIC UNCHOKE - We are adding a random peer to our top 4")
                        # print("BEFORE")
                        # print(top_four)
                        top_four[3].am_choking == 1
                        top_four = top_four[0:2]
            ...
            # print("*************OUR TOP 4 AFTER UPDATES/OPTIMISTIC UNCHOKE**********")
            # print(top_four)
