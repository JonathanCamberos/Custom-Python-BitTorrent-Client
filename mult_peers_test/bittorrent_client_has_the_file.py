...
def validate_peer_list():
    ...
    if len(client_state_list) > num_of_peers_threshold > len(new_clientlist):
        ...
        new_peers = tracker.get_peers_list(peer_id, min_port)
...
def send_from_file(client_we_are_serving, piece_name, piece_index, inner_offset, size_of_block, tracker):
    ...
    print("Inside send send send send send send from file")
    print("Sending piece: ", piece_index)
    ...
    total_offset = piece_starting_offset + inner_offset

    print("Piece starting offset: ", piece_starting_offset)
    print("Inner offset: ", inner_offset)
...
def handle_request_piece(client_we_are_serving, index, begin, length, tracker):
    ...
    print(f"! in handle_request_piece --> length = {length}")
    print(f"! in handle_request_piece --> index = {index}")
...
def start_dictionary_and_file(total_pieces):
    ...
    file = open("./has_the_file_file.txt", "r+b")
    ...
    if(size_res == 1):
        ...
        if (piece_index == total_num_pieces - 1):
            ...
            if (last_piece_size - last_piece_size_requested) >= piece_len_per_request:   #if size of block we need, is larger than max request size, request max size
                ...
                length = struct.pack(f"!{leftover}s", leftover)  #how much bytes are left for last piece (total_size - what we have recieved)                
...
def initialize_client_state_list():
    ...
    for p in peers:
        ...
        if args.ip_addr is None or args.ip_port is None:
...
if __name__ == '__main__':
    ...
    peers = tracker.get_peers_list(peer_id, min_port)
    if len(peers) == 0:
    ...
    my_bitfield = 0b11111110  # 7 '1's and 1 '0'
    my_bitfield_lst = convert_bitfield_to_list(my_bitfield)

    print("Sender bit Field --------> ", end=" ")
    print("{:b}".format(my_bitfield))

    print("Sender bit Field list ----> ", end=" ")
    print(my_bitfield_lst)
            ...
            for r in rfds:
                ...
                if (message_length == 0): #Keep Alive Message
                    ...
                    if id_num == 1: #Unchoke Message
                        ...
                        piece_index = struct.unpack("!I",piece_index)
                        client_we_are_serving.pieces_they_have.append(piece_index[0])  #update their bitifled
                ...
                if "optimistic_unchoke" in args.choking_strategy and (time_after_select - time_before_select).total_seconds() >= 30:
