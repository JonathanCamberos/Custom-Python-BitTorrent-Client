...
def check_size(piece_index, piece_size, total_pieces):
    ...
    if(piece_name == -1):
        ...
        print("**chech_size** Len: ", len(piece_dictionary[piece_name]))
...
def initialize_client_state_list():
    ...
    direct_peer = Peer("Unknown", "127.0.0.1", 6861, -1)
    
    #args.ip_addr  args.ip_port:
    sd = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        
    try:
        print(f"Trying to connect to 127.0.0.1:{6861}")
        sd = socket.create_connection(("127.0.0.1", 6861), timeout=1)
        sd.settimeout(None)
        print(f"! Connected to 127.0.0.1:{6861}")
            ...
            for r in rfds:
                ...
                if (message_length == 0): #Keep Alive Message
                    ...
                    if id_num == 1: #Unchoke Message
                        ...
                        print(f"Got piece with idx={index}")
