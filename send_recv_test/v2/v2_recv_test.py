...
def compare_bitfield_and_request_piece(peer, piece_length):
    print(" ##### Comparing Bitfield and Request Method #####")
...
def start_dictionary_and_file(total_pieces):
    ...
    file = open("v2_recv_test_file", "r+b")
...
def add_bytes_to_dictionary(bytes, piece_index, total_piece_size, offset, client_we_are_serving, total_num_pieces, tracker):
    ...
    if(piece_name == -1):
        ...
        print("Tset something different, and so it is")
        test = generate_hash_from_bytes(bytes)
        print("Adding --- > ", test)

        #appending bytes recieved to piece byteArray
        piece_dictionary[piece_name].extend(bytes)
        print("Dictionary curr Size after ADD TO: ", len(piece_dictionary[piece_name]))
        print("Expected total size of (total_piece_size): ", total_piece_size)
...
def generate_hash_from_bytes(bytes):
    ...
    print("INSIDE GENERATE HASH FROM BYTES")
    print("Len of Bytes passed to func ---> ", len(bytes), " Of type --> ", type(bytes))
    
    bencoded_bytes = bencode.encode(bytes)
    #print("Type of Bencoded Bytes: ", type(bencoded_bytes))
    #print("bencoded bytes : ", bencoded_bytes)

    hashed_bytes = hashlib.sha1(bencoded_bytes)
    #print("Type of Sha1'ed bytes: ", type(hashed_bytes))
    #print("sha1 on previous: ", hashed_bytes)

    #digested_bytes = hashed_bytes.digest()
    #print(".digest() on previous: ", digested_bytes)

    digested_bytes = hashed_bytes.digest()
    #print("Type of digested_bytes: ", type(digested_bytes))
    #print(".digest() on previous: ", digested_bytes)
    
    hash = digested_bytes
    #hash = hashlib.sha1(bytes).digest()

    print("Len of return hash: ", len(hash))
    return hash
...
def check_hash(piece_name, piece_index, total_piece_size, client_we_are_serving, num_pieces, tracker):
    ...
    print("Correct hash type: ", type(correct_hash))
    ...
    if(correct_hash == test_hash):
        print("HEHEHEHEHEHEHEHEHEHEHEHEHEHEHEHEHEH")
        time.sleep(1)
...
def check_size(piece_index, piece_size, total_pieces):
    ...
    if(piece_name == -1):
        ...
        #looking at the last piece
        if(piece_index == total_pieces-1):

            if(len(piece_dictionary[piece_name]) ==  last_piece_size):
                # print("All blocks for last piece Recieved!!")
                return 1
            elif(len(piece_dictionary[piece_name]) >  last_piece_size):
                #print("Too many blocks for last piece recieved!! ---> Reseting...")
                return -1
            else:
            ...
            for r in rfds:
                ...
                if (message_length == 0): #Keep Alive Message
                    ...
                    if id_num == 1: #Unchoke Message
                        ...
                        print(" 1111    Recieved a bitfield!!   1111111111 ")
                        ...
                        print("Inside R E C I E V E a Block/Piece MEssage  ++++++++++++++++++++")
