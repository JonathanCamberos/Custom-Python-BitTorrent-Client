...
def recv_handshake_from_initiator(my_socket, my_peerid):
    ...
    print("Someone is trying to talk to us from the void!! A new friend!! :)")
    ...
    print("Connection accepted!!! on peer sock: ", peer_sock)
...
def start_dictionary_and_file(total_pieces):
    ...
    file = open("v2_send_test_file", "r+b")
    #file = open("send_test_file", "w+b")
...
def initialize_client_state_list():
    ...
    for p in peers:
        ...
        if args.ip_addr == None and args.ip_port == None:
        ...
        #     port += 1
        # print("BitTorrent client bound to port " + str(port))
        try:
...
if __name__ == '__main__':
    ...
    # 3. Connect to the pemy_bitfielders using TCP to download the file
    ...
    while True:
        ...
        if rfds != []:
            for r in rfds:
                ...
                if (r == my_socket):
                    #recv_message(my_socket, peer_id)
